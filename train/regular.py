...
def train(train_data, val_data, model, args):
    ...
    for ep in range(args.train_epochs):
        ...
        for task in sampled_tasks:
            if task is None:
                break
            loss1 = train_one(task, model, opt, args, grad)
            loss.append(loss1)
    ...
    if args.save:
        ...
        out_dir = os.path.abspath(os.path.join(
                                      os.path.curdir,
                                      "saved-runs",
                                      args.embedding + '_' + args.classifier + '_' + args.dataset))
        if not os.path.exists(out_dir):
            os.makedirs(out_dir)
...
def train_one(task, model, opt, args, grad):
    ...
    if (args.classifier == "mn" or args.classifier == "contrastnet") and args.embedding == "ebd":
        XS, XS_aug = model['ebd'](support, flag='support')
        YS = support['label']

        XQ, XQ_aug = model['ebd'](query)
        YQ = query['label']

        _, loss = model['clf'](XS, YS, XQ, YQ, XS_aug, XQ_aug)
...
def test_one(task, model, args):
    ...
    if (args.classifier == "mn" or args.classifier == "contrastnet") and args.embedding == "ebd":
        ...
        acc, _ = model['clf'](XS, YS, XQ, YQ, XS_aug, XQ_aug)
    ...
    index, _ = torch.unique(YQ, sorted=True, return_inverse=True)

    return acc

def f1_score(y_ture, y_pred, label):
